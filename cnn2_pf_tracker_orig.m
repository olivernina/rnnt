function  position = cnn2_pf_tracker_orig(tracker_param)
...
%         tlength = 15;
%         if im2_id > tlength
%             [pred,terror] = lstm_pred(position(:,im2_id-tlength:im2_id));
%             lstm_geo = best_geo_param;
%             lstm_geo(1:2,:) = pred(1:2,:);
%             location = affgeo2loc(lstm_geo, pf_param.p_sz);
%             if terror > 0.0001
%                 drawresultg(im2_id, mat2gray(im2), [pf_param.p_sz, pf_param.p_sz], affparam2mat(lstm_geo));
%             end
% 
%             best_geo_param = lstm_geo;
...
%         %%%retrain because confidence is low
%         caffe('set_phase_train');
...
%         caffe('reshape_input', 'ssolver', [0, 1, length(lid), fea_sz(2), fea_sz(1)]);
    ...
    update_thr = 20 %original 20
    
    if conf_store>pf_param.up_thr && mod(im2_id,update_thr) == 0 % && ~l_distractor_store
        caffe('set_phase_train');
        caffe('reshape_input', 'ssolver', [0, 2, length(lid), fea_sz(2), fea_sz(1)]);
...
%% save results
save([tracker_param.result_path '/position.mat'], 'position');
end
